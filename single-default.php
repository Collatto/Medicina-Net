	...
	<main id="main" class="m-all t-2of3 d-5of7 cf" role="main" itemscope itemprop="mainContentOfPage" itemtype="http://schema.org/Blog">
		...
		<?php 
			...
			<?php if ($gambiQuery->have_posts()) : while ($gambiQuery->have_posts()) : $gambiQuery->the_post(); ?>
				...
				$codeUl .= '<ul>
				<li>
				<a href="'.get_permalink().'">'.char_formatter(get_the_title()).'</a>
				</li>
				</ul>';	
				?>
			<?php endwhile; endif; ?>
										...
										if(screen.width > 768){
											...
											for(let i = 0; i < pElement.length; i++){
												if(pElement[i].offsetTop - tempSBtags.offsetTop > 550){											
													...
													tempSBtags = document.getElementsByClassName('ad-sb')[count];
												}
											}		
										}else{
											let count = 1;
											...
											for(let i = 0; i < pElement.length; i++){
												if(pElement[i].offsetTop - tempLMBtags.offsetTop > 550){
													...
													tempLMBtags = document.getElementsByClassName('ad-lmb')[count];
												}
											}
										}
									}else{
										...
										if(screen.width > 768){
											...
											for(let i = 0; i < pElement.length; i++){
												if(pElement[i].offsetTop - tempLMBtags.offsetTop > 550){									
													...
													console.log("Insere div de posicionamento!");
													//divContent.insertBefore(adCode, pElement[i].nextElementSibling);
													pElement[i].innerHTML += '<hr style="opacity: 0.3;"><br><div class="ad-mobile" style="width: fit-content; margin: 0 auto;">										<div class="ad-lmb alright-ad" style="height:fit-content;width:320px;"></div></div><br><hr style="opacity: 0.3;">';
													tempLMBtags = document.getElementsByClassName('ad-lmb')[count];
												}
